...
### 기본 레이아웃 (13x9 직사각형 격자)
기본 설정은 맵 중앙에 상하 벽까지 완벽하게 이어지는 두 개의 파란색 장벽을 배치하고, 정중앙에 정확히 1칸(0.2m) 폭의 통과 구멍(Gap)을 제공하는 13x9 레이아웃입니다. 에이전트는 맵의 맨 왼쪽 벽 근처에 스폰되고, C자형 골 슬롯은 맨 오른쪽 벽 근처에 배치되어 넓은 탐색 반경과 OOD(Out-of-Distribution) 데이터를 유도합니다:
```python
grid = [
    "WWWWWWWWWWWWW",
    "W...........W",
...
* **truncated (시간 초과 중단):** 현재 스텝 수가 에피소드 제한 규격인 `max_episode_steps` (기본값: `150`)에 도달했을 때 `True`가 됩니다.
...
* 거리 및 회전 기반 클램핑(Clamping)이 적용되어 있어 조작 타겟과 실제 로봇의 위치 오차가 최대 `0.08m` 및 `30도`로 제한되어 현실적인 물리 충돌을 구현합니다.
