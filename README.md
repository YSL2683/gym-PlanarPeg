...
The default configuration places two blue barriers in the middle of the map that connect perfectly to the top and bottom walls, providing exactly a `0.09m` gap in the center. The agent spawns near the far-left wall, and the C-shaped goal slot is placed near the far-right wall to induce a wide exploration radius and Out-of-Distribution (OOD) data:
```python
grid = [
    "WWWWWWWWWWW",
    "W.........W",
...
### Starting State
On every episode reset, the agent is placed following these rules:
1. The starting cell `S` on the grid map is converted to Cartesian $(x,y)$ coordinates.
2. A random position noise sampled from a uniform distribution of **`[-0.05, 0.05]` meters** is added to the initial coordinate.
3. A random orientation (yaw) noise sampled from a uniform distribution of **`[-10.0°, 10.0°]`** ($[-0.174, 0.174]\text{rad}$) is added.
4. The control mocap target body is perfectly synchronized with this randomized spawn pose to prevent physical jerks.
...
#### 1. Registration Test
Test if the environment is properly registered in Gymnasium:
```bash
...
Operate the agent in real-time using a keyboard to test the environment mechanics **without saving data**:
```bash
python scripts/teleop.py
```
* **Controls**: `W/S` (Up/Down), `A/D` (Left/Right), `Q/E` (Rotate CCW/CW), `R` (Manual Reset), `ESC` (Quit)
...
기본 설정은 맵 중앙에 상하 벽까지 완벽하게 이어지는 두 개의 파란색 장벽을 배치하고, 정중앙에 정확히 `0.09m` 폭의 통과 구멍(Gap)을 제공하는 11x9 레이아웃입니다. 에이전트는 맵의 맨 왼쪽 벽 근처에 스폰되고, C자형 골 슬롯은 맨 오른쪽 벽 근처에 배치되어 넓은 탐색 반경과 OOD(Out-of-Distribution) 데이터를 유도합니다:
```python
grid = [
    "WWWWWWWWWWW",
    "W.........W",
...
4. 조종용 모캡(Mocap) 타겟 바디는 리셋 직후 이 무작위화된 스폰 포즈와 완벽하게 동기화(Sync)되어 물리적 튀는 현상을 방지합니다.
...
#### 2. 테스트 조종 (Play Mode)
**데이터 저장 없이** 키보드를 사용하여 에이전트를 실시간으로 조작하며 환경의 동작 상태와 조작감을 점검할 수 있습니다:
```bash
python scripts/teleop.py
```
* **단축키**: `W/S` (상/하), `A/D` (좌/우), `Q/E` (회전), `R` (현재 에피소드 수동 리셋), `ESC` (종료)
