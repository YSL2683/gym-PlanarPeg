...
* `image_top`: 미로 전체를 위에서 내려다보는 탑뷰 RGB 이미지 (`Box(0, 255, (224, 224, 3), dtype=np.uint8)`). (사전학습된 비전 모델에 완벽히 호환되는 고해상도 정사각형 비율입니다.)
* `image_front`: 직사각형 에이전트의 전면부에 부착되어 전방을 바라보는 1인칭 전방 뷰 RGB 이미지 (`Box(0, 255, (224, 224, 3), dtype=np.uint8)`).
* `proprioception`: 직사각형 에이전트의 실제 월드 절대 물리 포즈를 나타내는 3차원 상태 배열 (`Box(-inf, inf, (3,), dtype=np.float32)`):
...
* **키보드 조작 단축키**:
  * `W` / `S`: 전역 좌표계(Global) 기준 **위 / 아래 이동** (월드 Y축)
  * `A` / `D`: 전역 좌표계(Global) 기준 **좌 / 우 이동** (월드 X축)
  * `Q` / `E`: 제자리에서 **반시계방향(좌) / 시계방향(우) 회전** (Yaw각 스핀)
...
| :--- | :--- | :---: | :--- |
| `obs/image_top` | $(N, 224, 224, 3)$ | `uint8` | 탑뷰(Top-view) RGB 이미지 관측값. (Gzip 압축 적용) |
| `obs/image_front` | $(N, 224, 224, 3)$ | `uint8` | 에이전트 전면에 장착된 1인칭 전방뷰 RGB 이미지 관측값. (Gzip 압축 적용) |
| `obs/proprioception` | $(N, 3)$ | `float32` | 에이전트의 실제 3-DoF 물리 상태 `[X, Y, Theta]`. ($X, Y$: 절대 월드 좌표 미터 단위, $Theta$: Z축 기준 절대 라디안 각도 $[-\pi, \pi]$) |
...
### 2. 시연 데이터 뷰어 (Viewer)
수집된 데이터 파일이 모델에 입력되기 전 상태를 육안으로 검증할 수 있는 순수 OpenCV 기반의 경량 뷰어 스크립트가 제공됩니다.
```bash
python scripts/view_data.py --file data/teleop_demos/demo_[Timestamp].hdf5
```
* **기능:** 탑뷰와 프론트뷰 이미지를 동기화하여 재생하며, 에이전트의 실제 물리 포즈(Proprioception, 실선)와 사용자의 입력 조작(Action, 점선)을 동일한 스케일(`[-1, 1]`)로 정규화하여 실시간 그래프로 비교해줍니다. (스페이스바로 일시정지 및 A/D 키로 프레임 단위 정밀 탐색 가능)

### 3. 메타데이터 속성 (Attributes)
